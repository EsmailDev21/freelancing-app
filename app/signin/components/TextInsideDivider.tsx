...
const TextInsideDivider = ({text}:{text:TextObject}) => {
  return (
    <View marginY={5} width={"100%"} display={"flex"} flexDir={"row"} alignItems={"center"} justifyContent={"center"}>
      <Divider h={"2px"} width={"30%"} color={"muted.700"} />
      <Translator marginX={2} width={"24"} color={"muted.700"} text={text}></Translator>
      <Divider h={"2px"} width={"30%"} color={"muted.700"} />
    </View>
  )
}
